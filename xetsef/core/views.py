...
def feed(request):#tis is fnction for login and log ouut userdst o see the feed type
...
def change_username(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            update_session_auth_hash(request, user)  #update the server type to make the user to be log in so that cliet doesnot get logged out each time
